...
import static old.mame.cpuintrf.cpu_gettotalcpu;
import static old.mame.cpuintrf.cpuintf;
import static old.mame.cpuintrfH.burnPtr;
...
public class timer {
    ...
    */
    public static double getabsolutetime() {
        if (activecpu != -1 && (cpudata[activecpu].icount[0] + cpudata[activecpu].lost) > 0)
 ...
 */
    public static void timer_init() {
        ...
        for (int i = 0; i <= lastcpu; i++)//for (cpu_old = cpudata, i = 0; cpu_old <= lastcpu; cpu_old++, i++)
        {
 ...
 */
    ...
    public static timer_entry timer_pulse(double period, int param, timer_callback callback) {
        ...
        if (activecpu != -1 && timer.expire < base_time)
 ...
 */
    ...
    public static void timer_update_cpu(int cpunum, int ran) {
        ...
        //verbose_print("T=%.6g: CPU %d finished (net=%d)\n", cpu_old->time + global_offset, cpunum, ran - cpu_old->lost);
        /* time to renormalize? */
        if (cpu.time >= 1.0) {
            timer_entry timer;
            double one = 1.0;
    ...
    public static void timer_suspendcpu(int cpunum, int suspend, int reason) {
        //cpu_entry *cpu_old = cpudata + cpunum;
        ...
        if (activecpu != -1 && cpunum == activecpu && old == 0 && cpudata[cpunum].suspended != 0) {
    ...
    public static void timer_trigger(int trigger) {
        ...
        for (cpu = 0; cpu <= lastcpu; cpu++)//for (cpu_old = cpudata; cpu_old <= lastcpu; cpu_old++)
        {
            if (cpudata[cpu].suspended != 0 && cpudata[cpu].trigger == trigger) {
                //verbose_print("T=%.6g: CPU %d triggered\n", getabsolutetime() + global_offset, cpu_old->index);
    ...
    /*			if (!cpu_old->nocount)
            ...
            /* if this CPU isn't suspended and has time left.... */
                ...
                if (cycles[0] > 0) {
                    //verbose_print("T=%.6g: CPU %d runs %d cycles\n", cpu_old->time + global_offset, *cpunum, *cycles);
