...
UBytePtr mfm_internal_image_alloc(int num_tracks, int num_sides, DENSITY density);
/* get pointer to track data in internal image */
UBytePtr mfm_internal_image_get_track_ptr(UBytePtr image_data, int track, int side);


/* put a id to the track buffer */
void	mfm_put_id(UBytePtr buffer, unsigned char c, unsigned char h, unsigned char r, unsigned char n);

/* put a block of data to the track buffer */
void mfm_put_data(UBytePtr buffer, unsigned char data_id, UBytePtr data, unsigned long size);
