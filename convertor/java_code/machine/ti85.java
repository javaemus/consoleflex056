...
public class ti85
{
	...
	public static timer_callback ti85_timer_callback = new timer_callback() { public void handler(int param) 
	{
		if (readinputport(8)&0x01)
		{
			...
			ti85_ON_pressed = 0;
		if (ti85_timer_interrupt_mask != 0)
		{
			cpu_set_irq_line(0,0, HOLD_LINE);
	...
	{
		if ((ti85_memory_page_0x4000 & 0x40) != 0)
		{
		...
		if ((ti86_memory_page_0x8000 & 0x40) != 0)
		{
	...
	public static InitMachinePtr ti81_init_machine = new InitMachinePtr() { public void handler() 
	{
	...
	public static InitMachinePtr ti85_init_machine = new InitMachinePtr() { public void handler() 
	{
		ti85_timer_interrupt_mask = 0;
		ti85_timer_interrupt_status = 0;
		ti85_ON_interrupt_mask = 0;
	...
	public static InitMachinePtr ti86_init_machine = new InitMachinePtr() { public void handler() 
	{
		ti85_timer_interrupt_mask = 0;
		ti85_timer_interrupt_status = 0;
		ti85_ON_interrupt_mask = 0;
	...
	READ_HANDLER ( ti85_port_0003_r )
	{
		...
		if (ti85_LCD_status != 0)
			data |= ti85_LCD_mask;
		if (ti85_ON_interrupt_status != 0)
			data |= 0x01;
		if (ti85_timer_interrupt_status != 0)
			data |= 0x04;
		if (!ti85_ON_pressed)
			data |= 0x08;
		ti85_ON_interrupt_status = 0;
	...
	public static nvramPtr ti81_nvram_handler  = new nvramPtr() { public void handler(Object file, int read_or_write) 
	{
		if (read_or_write != 0)
	...
	public static nvramPtr ti85_nvram_handler  = new nvramPtr() { public void handler(Object file, int read_or_write) 
	{
		if (read_or_write != 0)
			osd_fwrite(file, memory_region(REGION_CPU1)+0x8000, sizeof(unsigned char)*0x8000);
		else
	...
	public static nvramPtr ti86_nvram_handler  = new nvramPtr() { public void handler(Object file, int read_or_write) 
	{
		if (read_or_write != 0)
		{
			if (ti86_ram != 0)
		...
		{
			ti86_ram = (UBytePtr )malloc(128*1024);
			if (ti86_ram != 0)
			{
				if (file != 0)
				{
	...
	static void ti85_setup_sav(UBytePtr data, unsigned long data_size)
	{
		int i;
		unsigned char lo,hi;
		UBytePtr  reg = data + 0x40;
		UBytePtr  hdw = data + 0x8000 + 0x94;
		...
		if (ti85_LCD_status != 0) ti85_LCD_mask = 0x02;
	...
	static void ti86_setup_sav(UBytePtr data, unsigned long data_size)
	{
		unsigned char lo,hi;
		UBytePtr  reg = data + 0x40;
		UBytePtr  hdw = data + 0x20000 + 0x94;
