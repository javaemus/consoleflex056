...
public class oric
{
	...
	*/
	...
	public static InitMachinePtr oric_common_init_machine = new InitMachinePtr() { public void handler() 
	{
	    /* clear all irqs */
		oric_irqs = 0;
	...
	void oric_shutdown_machine (void)
	{
		...
		if (oric_ram_0x0c000 != 0)
			free(oric_ram_0x0c000);
		oric_ram_0x0c000 = NULL;
		wd179x_exit();
	
		if (oric_tape_timer != 0)
		{
	...
	*/
	...
	public static InitMachinePtr telestrat_init_machine = new InitMachinePtr() { public void handler() 
	{
		oric_common_init_machine();
		...
		telestrat_blocks[0].ptr = (UBytePtr )malloc(16384);
	
		telestrat_blocks[1].MemType = TELESTRAT_MEM_BLOCK_RAM;
		telestrat_blocks[1].ptr = (UBytePtr )malloc(16384);
		telestrat_blocks[2].MemType = TELESTRAT_MEM_BLOCK_RAM;
		telestrat_blocks[2].ptr = (UBytePtr )malloc(16384);
		...
		telestrat_blocks[4].ptr = (UBytePtr )malloc(16384);
