		...
		switch( offset )
		{
			...
			case 2:
				...
				if (This.device.handshake_out) {
					int lines=0;
					if ((data & CONTROL_SELECT) != 0) lines|=CENTRONICS_SELECT;
					if ((data & CONTROL_NO_RESET) != 0) lines|=CENTRONICS_NO_RESET;
					if ((data & CONTROL_AUTOLINEFEED) != 0) lines|=CENTRONICS_AUTOLINEFEED;
					if ((data & CONTROL_STROBE) != 0) lines|=CENTRONICS_STROBE;
		...
		switch( offset )
		{
			...
			case 1:
				if (This.device&&This.device.handshake_in) {
					...
					data=0;
					if ((lines & CENTRONICS_NOT_BUSY) != 0) data|=STATUS_NOT_BUSY;
					if ((lines & CENTRONICS_ACKNOWLEDGE) != 0) data|=STATUS_ACKNOWLEDGE;
					if ((lines & CENTRONICS_NO_PAPER) != 0) data|=STATUS_NO_PAPER;
					if ((lines & CENTRONICS_ONLINE) != 0) data|=STATUS_ONLINE;
					if ((lines & CENTRONICS_NO_ERROR) != 0) data|=STATUS_NO_ERROR;
				} else {
					data=0x9c;
