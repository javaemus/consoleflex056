...
public class vc20
{
	...
	*/
	...
	public static ReadHandlerPtr vc20_via0_read_porta  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (JOYSTICK != 0) {
			if (JOYSTICK_BUTTON != 0) value&=~0x20;
			if (JOYSTICK_LEFT != 0) value&=~0x10;
			if (JOYSTICK_DOWN != 0) value&=~0x8;
			if (JOYSTICK_UP != 0) value&=~0x4;
		}
		if (PADDLES != 0) {
			if (PADDLE1_BUTTON != 0) value&=~0x10;
		}
	...
	public static ReadHandlerPtr vc20_via1_read_portb  = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (JOYSTICK != 0) {
			if (JOYSTICK_RIGHT != 0) value&=~0x80;
		}
		if (PADDLES != 0) {
			if (PADDLE2_BUTTON != 0) value&=~0x80;
		}
	...
	*/
	...
	public static InterruptPtr vc20_frame_interrupt = new InterruptPtr() { public int handler() 
	{
		...
		via_0_ca1_w (0, vc20_via0_read_ca1 (0));
		keyboard[0] = 0xff;
		if (KEY_DEL != 0) keyboard[0]&=~0x80;
		if (KEY_POUND != 0) keyboard[0]&=~0x40;
		if (KEY_PLUS != 0) keyboard[0]&=~0x20;
		if (KEY_9 != 0) keyboard[0]&=~0x10;
		if (KEY_7 != 0) keyboard[0]&=~0x08;
		if (KEY_5 != 0) keyboard[0]&=~0x04;
		if (KEY_3 != 0) keyboard[0]&=~0x02;
		if (KEY_1 != 0) keyboard[0]&=~0x01;
	
		keyboard[1] = 0xff;
		if (KEY_RETURN != 0) keyboard[1]&=~0x80;
		if (KEY_ASTERIX != 0) keyboard[1]&=~0x40;
		if (KEY_P != 0) keyboard[1]&=~0x20;
		if (KEY_I != 0) keyboard[1]&=~0x10;
		if (KEY_Y != 0) keyboard[1]&=~0x08;
		if (KEY_R != 0) keyboard[1]&=~0x04;
		if (KEY_W != 0) keyboard[1]&=~0x02;
		if (KEY_ARROW_LEFT != 0) keyboard[1]&=~0x01;
	
		keyboard[2] = 0xff;
		if (KEY_RIGHT != 0) keyboard[2]&=~0x80;
		if (KEY_SEMICOLON != 0) keyboard[2]&=~0x40;
		if (KEY_L != 0) keyboard[2]&=~0x20;
		if (KEY_J != 0) keyboard[2]&=~0x10;
		if (KEY_G != 0) keyboard[2]&=~0x08;
		if (KEY_D != 0) keyboard[2]&=~0x04;
		if (KEY_A != 0) keyboard[2]&=~0x02;
		if (KEY_CTRL != 0) keyboard[2]&=~0x01;
	
		keyboard[3] = 0xff;
		if (KEY_DOWN != 0) keyboard[3]&=~0x80;
		if (KEY_SLASH != 0) keyboard[3]&=~0x40;
		if (KEY_COMMA != 0) keyboard[3]&=~0x20;
		if (KEY_N != 0) keyboard[3]&=~0x10;
		if (KEY_V != 0) keyboard[3]&=~0x08;
		if (KEY_X != 0) keyboard[3]&=~0x04;
		if (KEY_LEFT_SHIFT != 0) keyboard[3]&=~0x02;
		if (KEY_STOP != 0) keyboard[3]&=~0x01;
	
		keyboard[4] = 0xff;
		if (KEY_F1 != 0) keyboard[4]&=~0x80;
		if (KEY_RIGHT_SHIFT != 0) keyboard[4]&=~0x40;
		if (KEY_POINT != 0) keyboard[4]&=~0x20;
		if (KEY_M != 0) keyboard[4]&=~0x10;
		if (KEY_B != 0) keyboard[4]&=~0x08;
		if (KEY_C != 0) keyboard[4]&=~0x04;
		if (KEY_Z != 0) keyboard[4]&=~0x02;
		if (KEY_SPACE != 0) keyboard[4]&=~0x01;
	
		keyboard[5] = 0xff;
		if (KEY_F3 != 0) keyboard[5]&=~0x80;
		if (KEY_EQUALS != 0) keyboard[5]&=~0x40;
		if (KEY_COLON != 0) keyboard[5]&=~0x20;
		if (KEY_K != 0) keyboard[5]&=~0x10;
		if (KEY_H != 0) keyboard[5]&=~0x08;
		if (KEY_F != 0) keyboard[5]&=~0x04;
		if (KEY_S != 0) keyboard[5]&=~0x02;
		if (KEY_CBM != 0) keyboard[5]&=~0x01;
	
		keyboard[6] = 0xff;
		if (KEY_F5 != 0) keyboard[6]&=~0x80;
		if (KEY_ARROW_UP != 0) keyboard[6]&=~0x40;
		if (KEY_AT != 0) keyboard[6]&=~0x20;
		if (KEY_O != 0) keyboard[6]&=~0x10;
		if (KEY_U != 0) keyboard[6]&=~0x08;
		if (KEY_T != 0) keyboard[6]&=~0x04;
		if (KEY_E != 0) keyboard[6]&=~0x02;
		if (KEY_Q != 0) keyboard[6]&=~0x01;
	
		keyboard[7] = 0xff;
		if (KEY_F7 != 0) keyboard[7]&=~0x80;
		if (KEY_HOME != 0) keyboard[7]&=~0x40;
		if (KEY_MINUS != 0) keyboard[7]&=~0x20;
		if (KEY_0 != 0) keyboard[7]&=~0x10;
		if (KEY_8 != 0) keyboard[7]&=~0x08;
		if (KEY_6 != 0) keyboard[7]&=~0x04;
		if (KEY_4 != 0) keyboard[7]&=~0x02;
		if (KEY_2 != 0) keyboard[7]&=~0x01;
