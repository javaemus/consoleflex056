...
public class atom
{
	...
	/* load image */
	int atom_load(int type, int id, UBytePtr *ptr)
	{
		void *file;
	...
	int atom_init_atm (int id)
	{
		...
		if (atom_load(IO_QUICKLOAD, id, &quickload_data))
		{
			if (quickload_data!=NULL)
			{
				...
				data = (UBytePtr )((unsigned long)quickload_data + sizeof(struct atm));
	...
	*/
	...
	public static InitMachinePtr atomeb_init_machine = new InitMachinePtr() { public void handler() 
	{
		atom_init_machine();
		atom_eprom_box_init();
	} };
