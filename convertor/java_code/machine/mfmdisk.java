...
public class mfmdisk
{
	...
	*/
	...
	static void mfm_info_cache_sector_info(int id,UBytePtr pTrackPtr, unsigned long Length)
	{
	...
	static UBytePtr mfm_disk_get_track_ptr(int id, int track, int side)
	{
		unsigned long TrackSize;
	
		TrackSize = mfm_disk_get_track_size(id);
	
		return (UBytePtr )((unsigned long)mfm_disks[id].pData + mfm_disk_header_size + (unsigned long)(TrackSize*((track*mfm_disks[id].NumSides)+side)));
	}
	
	/* this is endian safe */
	unsigned long mfm_get_long(UBytePtr addr)
	{
		int i;
	...
	/* load image */
	static int mfm_disk_load(int type, int id, UBytePtr *ptr)
	{
		void *file;
	...
	static void mfm_disk_cache_data(int drive, int Track, int Side)
	{
		...
		{
			UBytePtr pTrackPtr = mfm_disk_get_track_ptr(drive, Track,Side);
