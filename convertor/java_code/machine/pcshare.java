...
public class pcshare
{
	...
	#define DBG_LOG(N,M,A) \
		if(VERBOSE_DBG>=N){ if (M != 0)logerror("%11.6f: %-24s",timer_get_time(),(char*)M ); logerror A; }
	#else
	#define DBG_LOG(n,m,a)
	#endif
	...
	#define JOY_LOG(N,M,A) \
		if(VERBOSE_JOY>=N){ if (M != 0)logerror("%11.6f: %-24s",timer_get_time(),(char*)M ); logerror A; }
	#else
	#define JOY_LOG(n,m,a)
	#endif
