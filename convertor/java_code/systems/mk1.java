...
public class mk1
{
	...
	//    logerror ("f8 %.6f r %x %x\n", timer_get_time(), offset, data);
	    if (offset==0) {
		if ((data & 1) != 0) data|=readinputport(1);
		if ((data & 2) != 0) data|=readinputport(2);
		if ((data & 4) != 0) data|=readinputport(3);
		if ((data & 8) != 0) data|=readinputport(4);
		if ((data & 0x10) != 0) {
	...
	static public static InitMachinePtr mk1_machine_init = new InitMachinePtr() { public void handler() 
	{
	    f3853_reset();
	} };
	
	static MachineDriver machine_driver_mk1 = new MachineDriver
	(
		...
		sizeof (mk1_palette) / sizeof (mk1_palette[null]) + 32768,
		sizeof (mk1_colortable) / sizeof(mk1_colortable[null][null]),
	...
	public static InitDriverPtr init_mk1 = new InitDriverPtr() { public void handler() 
	{
	    F3853_CONFIG config;
