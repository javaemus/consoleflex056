...
public class exidy
{
	...
	*/
	...
	static READ_HANDLER(exidy_fd_port_r)
	{
		...
		/* bit 4: parity error */
		if ((exidy_hd6402_state & HD6402_OUTPUT_PE) != 0)
		{
			data |= (1<<4);
		}
	
		/* bit 3: framing error */
		if ((exidy_hd6402_state & HD6402_OUTPUT_FE) != 0)
		{
			data |= (1<<3);
		}
	
		/* bit 2: over-run error */
		if ((exidy_hd6402_state & HD6402_OUTPUT_OE) != 0)
		{
			data |= (1<<2);
		}
	
		/* bit 1: data receive ready - data ready in receive reg */
		if ((exidy_hd6402_state & HD6402_OUTPUT_DR) != 0)
		{
			data |= (1<<1);
		}
	
		/* bit 0: transmitter buffer receive empty */
		if ((exidy_hd6402_state & HD6402_OUTPUT_TBRE) != 0)
		{
			data |= (1<<0);
		}
	...
	static MachineDriver machine_driver_exidy = new MachineDriver
	(
		...
		new rectangle(0, (EXIDY_SCREEN_WIDTH - 1), 0, (EXIDY_SCREEN_HEIGHT - 1)),        /* rectangle: visible_area */
		null,								   /* graphics
											* decode info */
