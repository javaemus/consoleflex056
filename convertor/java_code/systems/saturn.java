...
public class saturn
{
	...
	*/
	  ...
	  if(offset < 0xA)
	    {
	      if(offset == CD_CR1)
		{
		  if (periodic != 0) /* Hack to indicate periodic response while cd system not finished */
		    {
		      periodic = 0;
	...
	void draw_1s8(UINT32 *vram_base,UBytePtr display,UINT32 pitch)
	...
	void render_plane(UBytePtr buffer,int pal,int trans)
	...
	public static InitMachinePtr saturn_init_machine = new InitMachinePtr() { public void handler() 
	{
	  int i;
	  UINT32 *mem2;
	  int mem_length;
	...
	public static InitDriverPtr init_saturn = new InitDriverPtr() { public void handler() 
	{
	  reset_smpc();
	  reset_cd();
	  reset_scu();
	...
	public static VhStartPtr saturn_vh_start = new VhStartPtr() { public int handler() 
	{
	...
	public static VhStopPtr saturn_vh_stop = new VhStopPtr() { public void handler() 
	{
	  if(saturn_bitmap[0])
	...
	*/
	...
	public static VhConvertColorPromPtr saturn_init_palette = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	...
	static MachineDriver machine_driver_saturn = new MachineDriver
	(
	  ...
	  new MachineCPU[] {
	    ...
	    new MachineCPU(
	      ...
	      null,null,                      /* zeros are ioport read/write */
	      ignore_interrupt,1
	    )
	  },
	  ...
	  new rectangle( 0, SATURN_SCR_WIDTH - 1, 0, SATURN_SCR_HEIGHT - 1),
	  NULL,
	  32768, 32768,
