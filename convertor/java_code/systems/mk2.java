...
public class mk2
{
	...
	*/
	...
	static public static InitMachinePtr mk2_machine_init = new InitMachinePtr() { public void handler() 
	{
		rriot_reset(0);
	} };
	
	static DACinterface mk2_dac = new DACinterface( 1, new int[] {80}); // silence is golden
	...
	(
		...
		sizeof (mk2_palette) / sizeof (mk2_palette[null]) + 32768,
		sizeof (mk2_colortable) / sizeof(mk2_colortable[null][null]),
		...
		0,0,0,0,
		new MachineSound[] {
			new MachineSound( SOUND_DAC, mk2_dac )
	    }
	);
	...
	*/
	...
	static int mk2_read_a(int chip)
	{
		...
		case 4:
			if ((help & 0x20) != 0) data&=~0x1; //F
			if ((help & 0x10) != 0) data&=~0x2; //E
			if ((help & 8) != 0) data&=~0x4; //D
			if ((help & 4) != 0) data&=~0x8; // C
			if ((help & 2) != 0) data&=~0x10; // B
			if ((help & 1) != 0) data&=~0x20; // A
	#if 0
		...
		case 5:
			...
			if ((help & 0x80) != 0) data&=~0x20; // H
			if ((help & 0x40) != 0) data&=~0x40; // G
			break;
		}
		return data;
	}
