...
public class arcadia
{
	...
	*/
	...
	public static VhStartPtr arcadia_vh_start = new VhStartPtr() { public int handler() 
	{
	    int i;
	    for (i=0; i<0x40; i++) {
		rectangle[i][0]=0;
		rectangle[i][4]=0;
		if ((i & 1) != 0) rectangle[i][0]|=3;
		if ((i & 2) != 0) rectangle[i][0]|=0x1c;
		if ((i & 4) != 0) rectangle[i][0]|=0xe0;
		if ((i & 8) != 0) rectangle[i][4]|=3;
		if ((i & 0x10) != 0) rectangle[i][4]|=0x1c;
		if ((i & 0x20) != 0) rectangle[i][4]|=0xe0;
	...
	public static VhStopPtr arcadia_vh_stop = new VhStopPtr() { public void handler() 
	{
	} };
				      ...
				      int y, int x)
	{
		...
		int c;
		if ((color & 0x40) != 0) c=arcadia_video.reg.d.pal[1];
		else c=arcadia_video.reg.d.pal[0];
		Machine.gfx[0].colortable[1]=Machine.pens[(c>>3)&7];
	
		if ((color & 0x80) != 0) c=arcadia_video.reg.d.pal[1];
		else c=arcadia_video.reg.d.pal[0];
