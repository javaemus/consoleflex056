...
public class pc8801
{
	...
	UBytePtr pc88sr_textRAM=NULL;
	int pc8801_is_24KHz;
	static UBytePtr gVRAM=NULL;
	...
	public static VhStartPtr pc8801_vh_start = new VhStartPtr() { public int handler() {return 0;} };
	...
	void pc8801_vh_refresh(struct mame_bitmap *bitmap,int full_refresh)
	{
	  ...
	  for(y=0;y<100;y++) {
		...
		if ((attr_new & TX_GL) != 0) {
		  ...
		  text_new>>=(text_new&3);
		  if ((attr_new & TX_REV) != 0) {
		    text_new=~text_new;
		  }
		  switch(attr_new&TX_WID_MASK) {
		  case TX_80:
		    if ((text_new & 0x04) != 0) plot_box(wbm2,x*8,y*BLOCK_YSIZE,4,BLOCK_YSIZE,ct);
		    if ((text_new & 0x40) != 0) plot_box(wbm2,x*8+4,y*BLOCK_YSIZE,4,BLOCK_YSIZE,ct);
		    break;
		  case TX_40L:
		    if ((text_new & 0x04) != 0) plot_box(wbm2,x*8,y*BLOCK_YSIZE,8,BLOCK_YSIZE,ct);
		    break;
		  case TX_40R:
		    if ((text_new & 0x40) != 0) plot_box(wbm2,x*8,y*BLOCK_YSIZE,8,BLOCK_YSIZE,ct);
		    break;
		  }
		} else {
		  /* normal text */
