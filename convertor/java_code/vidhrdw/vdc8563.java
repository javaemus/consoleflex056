...
public class vdc8563
{
	...
	#define DBG_LOG(N,M,A)      \
	    if(VERBOSE>=N){ if (M != 0)logerror("%11.6f: %-24s",timer_get_time(),(char*)M );
	 logerror A; }
	#else
	#define DBG_LOG(N,M,A)
	#endif
	...
	*/
	...
	public static VhStopPtr vdc8563_vh_stop = new VhStopPtr() { public void handler() 
	{
		free(vdc.ram);
	} };
	...
	void vdc8563_vh_screenrefresh (struct mame_bitmap *bitmap, int full_refresh)
	{
		...
		if (GRAPHIC != 0) { vdc8563_graphic_screenrefresh(bitmap, full_refresh);
		} else {
			for (i=0; i<512; i++) {
			...
			if (TEXT != 0) vdc8563_text_screenrefresh(bitmap, full_refresh);
