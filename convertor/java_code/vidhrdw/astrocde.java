...
public class astrocde
{
	...
	static final int FAKE_BLK = 0, FAKE_YLW = 1, FAKE_BLU = 2, FAKE_RED = 3, FAKE_WHT = 4;
	...
	public static VhConvertColorPromPtr astrocade_init_palette = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	{
	...
	//	logerror("%04x: magicram_w(%04x) = %02x, magic_register = %02x\n",cpu_getpc(),offset,data,magic_control);
		...
		if ((magic_control & EXPAND_MASK) != 0)	/* expand mode */
		{
			bits = data;
		...
		if ((magic_control & ROTATE_MASK) != 0)
		{
		...
		if ((magic_control & FLOP_MASK) != 0)	/* copy backwards */
		{
			bits = data;
			stib = 0;
			for (k = 0;k < 4;k++)
		...
		if ((magic_control & XOR_MASK) != 0)
			data ^= astrocade_videoram[offset];	    /* draw in XOR mode */
		else if ((magic_control & OR_MASK) != 0)
	...
	public static VhUpdatePtr astrocade_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
